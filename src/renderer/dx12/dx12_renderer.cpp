...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC* sampler_descriptors, UINT num_sampler_descriptors)
{
	...
	rs_descriptor.Init_1_1(_countof(root_parameters), root_parameters, num_sampler_descriptors, sampler_descriptors, rs_flags);
...
void cg::renderer::dx12_renderer::create_pso(const std::string& shader_name)
{
	...
	auto pixel_shader = compile_shader(
			get_shader_path(shader_name),
			"PSMain", "ps_5_0");
	auto pixel_shader_texture = compile_shader(
			get_shader_path(shader_name),
			"PSMain_texture", "ps_5_0");
	auto vertex_shader_shadow_map = compile_shader(
			get_shader_path(shader_name),
			"VSShadowMap", "vs_5_0"
	);
	...
	THROW_IF_FAILED(device->CreateGraphicsPipelineState(&pso_desc, IID_PPV_ARGS(&pipeline_state_shadow)));
}
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	CD3DX12_RANGE red_range(0,0);
	THROW_IF_FAILED(constant_buffer->Map(0, &red_range, reinterpret_cast<void**>(&constant_buffer_data_begin)));

	const size_t texture_num = model->get_per_shape_texture_files().size();

	cbv_srv_heap.create_heap(
			device, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV,
			static_cast<UINT>(3 + texture_num), D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE);
	...
	for (size_t i = 0; i < texture_num; i++)
	{
		...
		const UINT upload_buffer_size = static_cast<UINT>(GetRequiredIntermediateSize(textures[i].Get(), 0, 1));

		create_resource_on_upload_heap(upload_textures[i], upload_buffer_size);

		copy_data(image, upload_buffer_size,
			textures[i], upload_textures[i],
			D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE,
			tex_width * STBI_rgb_alpha,
			tex_height * tex_width * STBI_rgb_alpha);

		create_shader_resource_view(textures[i], cbv_srv_heap.get_cpu_descriptor_handle(
